import 'project_card.dart';

final List<Project> my_projects = [
  Project(
      'Five Streak',
      '''
    <p>A website where you try to keep a streak to help prevent the spread of COVID-19.</p>
    ''',
      'May 2020',
      'https://www.five-streak.com',
      'five-streak.png'),
  Project(
      'Git the lines',
      '''
    <p>A Discord bot that prints out the lines referenced in a GitHub or GitLab snippet link</p>
    ''',
      'May 2020',
      'https://github.com/dolphingarlic/git-the-lines',
      'git-the-lines.png'),
  Project(
      'PO(I)TD',
      '''
    <p>A Discord bot that sends a daily Olympiad Informatics problem</p>
    <p>Uses APIOIPA as the backend</p>
    ''',
      'April 2020',
      'https://github.com/dolphingarlic/PO-I-TD',
      'po-i-td.png'),
  Project(
      'APIOIPA',
      '''
    <p><b>API</b> for <b>O</b>lympiad <b>I</b>nformatics <b>P</b>roblems <b>A</b>ssorted</p>
    ''',
      'April 2020',
      'https://apioipa.herokuapp.com',
      'apioipa.png'),
  Project(
      'Polygrade',
      '''
    <p>A website that lets the user input their grades and view it as a radar chart</p>
    ''',
      'December 2019',
      'https://polygrade.herokuapp.com/',
      'polygrade.png'),
  Project(
      'Joining Points',
      '''
    <p>The game 'Joining Points' from IOI 2006 - a 1-player game where you try join points of different colours so that no lines intersect</p>
    ''',
      'November 2019',
      'https://github.com/dolphingarlic/joining_points',
      'joining_points.png'),
  Project(
      'SACO Editorials',
      '''
    <p>A collection of editorials for the South African Coding Olympiad that are not available elsewhere</p>
    <p>This will hopefully help others in preparation for the final round of SACO</p>
    ''',
      'October 2019',
      'https://github.com/dolphingarlic/SACO_editorials',
      'github.png'),
  Project(
      'Incredibowl',
      '''
    <p>A webiste that allows people to view, rate, and share bowls</p>
    ''',
      'August 2019',
      'https://incredibowl.herokuapp.com/',
      'incredibowl.png'),
  Project(
      'Accomplice in Coding',
      '''
    <p>A Google Assistant action that gives you random problems from informatics olympiads</p>
    ''',
      'July 2019',
      'https://assistant.google.com/services/a/uid/000000acca8b1c5d?hl=en&source=web',
      'ac_bot.png'),
];
